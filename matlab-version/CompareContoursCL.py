...
'''
CompareContours.py
Written by GHP on 9/24/2014 based on J. Berry's original script (CompareContours.py)

purpose:
	CL interface to CompareContours.py
	Runs Mean Sum of Distances (MSD) measurements on two sets of
	tongue traces and writes the results to the user-specified
	file

usage:
	python CompareContoursCL.py /path/to/gold/traces /path/to/experimental/trace /path/to/save/location"
...
class ContourComparer:
	...
	def __init__(self, gold_dir, exp_dir, save_dir):
		self.TRIM = False
		self.numGridlines = 32
		self.gold_dir = os.path.expanduser(gold_dir)
		self.exp_dir = os.path.expanduser(exp_dir)
		self.saveentry = save_dir
		self.machine = "Sonosite Titan"

		self.goldTraces = self.get_traces(self.gold_dir)
		self.experimentalTraces = self.get_traces(self.exp_dir)
		self.bad_traces = []
		self.threshold = 5

	def get_traces(self, somedirectory):
		return [trace for trace in os.path.listdir(somedirectory) if trace.endswith("*.traced.txt")]
	...
	def problem_traces(self):
		...
		problem_traces_dir = os.path.join(pathtosave, "{0}-PROBLEM_TRACES_MSDge{1}".format(os.path.basename(self.exp_dir, self.threshold)))
...
if __name__ == "__main__":
	if len(sys.argv) < 4:
		print "USAGE: python CompareContoursCL.py /path/to/gold/traces /path/to/experimental/trace /path/to/save/location"
		sys.exit(0)
	else:
		gd, ed, sd = sys.argv[1], sys.argv[2], sys.argv[3]

	cc = ContourComparer(gd, ed, sd)
	cc.compare_contours()
